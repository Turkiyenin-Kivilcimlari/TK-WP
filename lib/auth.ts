  ...
  callbacks: {
      ...
      if (credentials && !credentials.twoFactorCode) {
        ...
        try {
          ...
          if (dbUser?.twoFactorEnabled) {
            ...
            return `/signin?error=TwoFactorRequired&email=${encodeURIComponent(
              credentials.email as string
            )}`;
          }

          if (dbUser && !dbUser.emailVerified) {
            return `/signin?error=EmailNotVerified&email=${encodeURIComponent(
              credentials.email as string
            )}`;
          }
        } catch (error: any) {
    ...
    async jwt({ token, user, trigger, session }) {
      ...
      if (trigger === "update" && session) {
        ...
        if (session.user?.slug) token.slug = session.user.slug;
      }
  ...
  try {
    ...
    for (let i = -1; i <= 1; i++) {
      ...
      const hmacData = Buffer.from(
        timeCounter.toString(16).padStart(16, "0"),
        "hex"
      );
