  ...
  (error) => {
    ...
    if (error.response?.status === 401) {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (error.config?.url?.includes('/admin/') && error.response?.data?.errorType === '2fa_required') {
          // 2FA doğrulama sayfasına yönlendir
          window.location.href = '/admin/verify-2fa?returnUrl=' + encodeURIComponent(window.location.pathname);
          return Promise.reject(error);
        }
