...
import { createBackupRecord, updateBackupRecord } from '@/lib/backupRecordService';
...
export async function backupCloudinaryOnly(
  ...
  try {
    const basePath = process.env.BACKUP_PATH || path.join(process.cwd(), 'backups');
    
    // Yedekleme dizinini hazırla
    const { backupDir: finalBackupDir } = await prepareBackupDirectory(basePath);
    ...
    const cloudinaryBackupDir = path.join(
      targetDir,
      `cloudinary_backup_${new Date().toISOString().replace(/:/g, '-').split('.')[0]}`
    );
    ...
    console.log(`${assets.length} adet Cloudinary varlığı bulundu`);
    ...
    for (const asset of assets) {
      try {
        ...
        if (downloadedCount % 10 === 0) {
          console.log(`${downloadedCount}/${assets.length}: İlerleme devam ediyor...`);
        }
      } catch (assetError) {
        console.error(`Dosya indirme hatası (${asset.public_id}):`, assetError);
        failedCount++;
      }
    }
    
    console.log(`Cloudinary yedekleme tamamlandı: ${downloadedCount} başarılı, ${failedCount} başarısız`);
    
    // ZIP oluştur
    const zipFilePath = path.join(targetDir, `cloudinary_backup_${Date.now()}.zip`);
    ...
    if (encryptionKey) {
      ...
      console.log('Cloudinary yedeği şifrelendi');
    }
    ...
    console.error('Cloudinary yedeklemesi alınırken hata oluştu:', error);
    ...
    return { success: false, backupDir: backupDir || '', error: error.message };
  }
}
 ...
 */
async function downloadCloudinaryAsset(url: string, downloadPath: string): Promise<void> {
