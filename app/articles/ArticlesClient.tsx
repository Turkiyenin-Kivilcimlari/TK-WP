...
function ArticlesWithSearchParams({
  ...
  const getContentPreview = (article: Article) => {
    ...
    if (textBlocks.length > 0) {
      ...
      // (document yerine regex: bu bileşen SSR ile sunucuda da render ediliyor)
  ...
  useEffect(() => {
    // Sunucudan initialArticles geldiyse ilk listeyi yeniden çekmeye gerek yok;
    // arama/filtre/sayfalama etkileşimleri yine API'ye gider.
    if (!initialArticles) {
      fetchArticles();
    }
...
export default function ArticlesClient({
  ...
  return (
    <SearchParamsProvider>
      <ArticlesWithSearchParams initialArticles={initialArticles} />
    </SearchParamsProvider>
  );
}
