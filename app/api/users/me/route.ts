...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (kaggle !== undefined) updateData.kaggle = kaggle;
    if (huggingface !== undefined) updateData.huggingface = huggingface;
          ...
          return encryptedJson(
            {
              success: false,
              message:
                "Bu profil URL'si başka bir kullanıcı tarafından kullanılıyor",
            },
            { status: 400 }
          );
        }
          ...
          return encryptedJson(
            {
              success: false,
              message:
                "Profil URL'si yalnızca küçük harfler, sayılar ve tire içerebilir",
            },
            { status: 400 }
          );
        }
